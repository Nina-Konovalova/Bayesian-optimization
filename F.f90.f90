...
real*8     Ft(0:max_s) ! this is the actual total free energy profile
	
real*8     derF(0:max_s)  ! derivative of free energy by reaction coordinate
real*8     wmt(0:max_s, 0:max_t) ! W(s, t ) is the probability of a chain of length
...
real*8     dt !timestep ; note relationship to the friction coeff

real*8     u(1:1000) ! store result of triangular linear system solution
real*8     A(1:1000),B(1:1000),C(1:1000),D(1:1000) ! diagonals and the rhs of the same

real*8     zN(0:max_s),  pTimeN(0:max_t), pTime0(0:max_t) 
	! translocation and rejection time distributions (s reaches N and s reaches 0 respectively)


real*8    time,timeback ! average time of translocation and rejection
	
real*8    Translocation_Rate, pT, pF ! probabilities of successful and unsuccesful events

real*8    Jreturn(0:max_t), time_p_N(0:max_t) ,time_p_0(0:max_t) !w for s=N and s=0
...
write(2,'(a,i7,a,e13.6,a,e13.6)') &
        ' N = ',N,' success rate=',pT,' time=', time
write(2,'(a,i7,a,e13.6,a,e13.6)') &
        ' Nt = ',Nt,' failure rate=',1.0-pT,' time=', timeback
